...
export class SyncService {
  ...
  private async syncBoards(): Promise<boolean> {
    try {
      ...
      // 2. For each board, download its pictograms and SignWriting its next boards
      for (const board of boards) {
        try {
          const boardItems = await boardService.getBoardItems(board.uuid);

          if (boardItems && boardItems.length > 0) {
            const cacheKey = boardItemsCacheKey(board.uuid);
            await AsyncStorage.setItem(cacheKey, JSON.stringify(boardItems));
            console.log(`BoardItems synced for board: ${board.title}`);
          }
        } catch (picError) {
          // Catches isolated error from a specific board to avoid stopping the entire loop
          console.log(`Failed to sync BoardItems for board: ${board.uuid}`);
        }
